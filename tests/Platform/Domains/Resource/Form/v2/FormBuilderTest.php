...
class FormBuilderTest extends ResourceTestCase
{
    ...
    function test__builder_fields()
    {
        ...
        $this->assertEquals('name', $form->getField('users.fields.name')->getName());
        $this->assertEquals('email', $form->getField('users.fields.email')->getName());
    ...
    function test__resolves_field_values_from_initial_form_data()
    {
        $form = $this->makeFormBuilder($this->makeTestFields())
                     ->setFormData(['fields' => ['users.fields.name' => 'SuperV User']])
                     ->getForm();

        $this->assertEquals('SuperV User', $form->getFieldValue('users.fields.name'));
    }
